...
const CreditDelivery = () => {
    ...
    const fetchCreditDelivery = async (currentPage) => {
         const response = await axios.get(
           env.apiUrl +
             `api/users/stripeorderTodeliver?userId=${userID}&page=${currentPage}`,
           headers
         );
       
        if (response.data && response.data.data[0] && response.data.data[0].data &&  response.data.data[0].data.length >0) {
             setTotalRows(response.data.data[0].count[0].count);
            let addressesList = [];
    ...
    const handlePageChange = (page) => {
        setCurrentPage(page);
        fetchCreditDelivery(page);


    };

    const handlePerRowsChange = async (newPerPage, page) => {
        fetchCreditDelivery(page, newPerPage);
