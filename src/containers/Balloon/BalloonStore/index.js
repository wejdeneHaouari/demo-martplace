...
function BalloonStore() {
  ...
  const allHomeCerts = (page) => {
    ...
    history.push("/balloon/home?page=" + page);
    ...
    axios
      .get(
        env.apiUrl +
          getNftsbyUsername +
          `?username=balloonfestival&page=${page}`
      )
      .then((res) => {
  ...
  return (
    <>
      {/* {(storeOpen==true) && */}
      <div className="container balloonContainer">
        <Header />
        ...
        <div
          className="row balloon__homepage"
          style={{
            margin: "0 30px",
          }}
        >
