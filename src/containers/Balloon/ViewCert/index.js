...
function HomeViewCert() {
  ...
  const handleBtnClick = () => {
    if (token) {
      window.location.href = `/checkout?id=${certificateArray._id}`;
    } else {
  ...
  const allHomeCerts = () => {
    axios
      .get(env.apiUrl + getNftsbyUsername + `?username=balloonfestival`)
      .then((res) => {
          ...
          <div className="container viewContainer nftpdp__wrapper">
            <div className="nftpdp__cards-ctn">
              ...
              <div className="nftpdp__card nftpdp__card--content">
                ...
                {certificateArray.salesquantity &&
                  certificateArray.forSaleStatus && (
                    <button className="nftpdp__btn" onClick={handleBtnClick}>
                      Proceed To Checkout
                    </button>
                  )}
                {certificateArray.length !== 0 &&
                  !certificateArray.salesquantity &&
                  certificateArray.forSaleStatus && (
                    <button className="nftpdp__btn__soldout" disabled={true}>
                      Sold Out
                    </button>
                  )}

                <button
                  className="nftpdp__btn mt-4 w-50 btn btn-secondary"
                  onClick={redirectToFraction}
                >
                  Fraction NFT
                </button>
              </div>
            ...
            <div className="nftpdp__similar-items">
              <h1>Similar Items</h1>
              <Swiper
                spaceBetween={10}
                slidesPerView={2}
